const inputBusca = document.getElementByIds('busca');
const containe = document.getElementById('grid-musicas');
...
function exibirResultados(musicas){
    ...
    musicas.forEach(musica =>{
        ...
        const capUrl = musica.artworkUrl100.replace('100x100', '400x400');

        card.innerHTML =`
        <img src = "${capaUrl}" alt="${musica.trackName}">
        <h3>${musica.trackEname}</h3>
